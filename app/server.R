...
#list.of.packages <- c('shinyFiles','shinyMatrix','shinythemes','shinycssloaders','gdata', 'gplots','igraph', 'BiocManager')
#new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
#if(length(new.packages)) install.packages(new.packages)

install.packages(c('fs','shinyFiles','shinyMatrix','shinythemes','shinycssloaders','gdata', 'gplots','igraph', 'BiocManager'))
...
server <- function(input, output, session) {
  ...
  shinyFileChoose(input, 'DESCFILE', roots=c(wd='/data')) 
  shinyFileChoose(input, 'DATAFILE', roots=c(wd='/data')) 
  
  # Parse the desc file path and read the table
  desc <- reactive({
    validate(need(expr = input$DESCFILE, "Please select a description file"))
    descfile <- parseFilePaths(roots=c(wd='/data'), input$DESCFILE)
    if(NROW(descfile)) {
  ...
  readData <- reactive({
    ...
    readData <- parseFilePaths(roots=c(wd='/data'), input$DATAFILE)
  ...
  # Colors, symbols and dimensions for the plots
  colList <- reactive({
  ...
  output$controlplotaffx <- renderImage({
    ...
    if(!is.null(cntrl)) {
      ...
      if(length(affx1)>0 | length(grep("affx",cntrlVars))>0) {
        ...
        dev.off()
        list(src = RawDataAFFXControlsProfiles,
             width = WIDTH,
             height = HEIGHT,
             alt = "This is alternate text")
      }
  ...
  output$controlplotbox <- renderImage({
    ...
    list(src = RawDataAFFXControlsBoxplot,
         width = WIDTH,
         height = HEIGHT,
         alt = "This is alternate text")
  }, deleteFile = T)
  ...
  output$layoutplot<-renderImage({
    ...
    list(src = RawDataReferenceArrayLayout,
         width = WIDTH,
         height = HEIGHT,
         alt = "This is alternate text")
  }, deleteFile = T)
  ...
  output$posnegCOI<- renderImage({
    ...
    list(src = RawDataPosNegPositions,
         width = WIDTH,
         height = HEIGHT,
         alt = "This is alternate text")
  },deleteFile = T)
